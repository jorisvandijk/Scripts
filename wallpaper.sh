#!/bin/bash
#
# Usage: To create random wallpaper made with PotD 
#        from Wikipedia and a random English word
#
# Script by HexDSL | gitlab.com/hexdsl 
# Alterations by Joris van Dijk | gitlab.com/jorisvandijk
#
...
nowtime=$(date +%s)
count=0
basedir="/home/joris/Pictures/Wallpapers"
currun=0
numruns=1
imagename="image"
	...
	then
		...
		name="$basedir/$imagename$currun.jpg";
			...
			if [ $currun -ge $numruns ];then
				feh --bg-fill "$basedir/image1.jpg"
				exit;
			else
				getpic
			fi
